...
export default function GoogleOAuth({
  ...
  return (
    <button
      onClick={() => login()}
      disabled={isPending}
      className="w-full flex items-center justify-center gap-3 bg-white hover:bg-gray-50 disabled:opacity-60 disabled:cursor-not-allowed text-gray-700 font-medium text-sm px-6 py-3 rounded-xl border border-gray-200 shadow-sm hover:shadow transition-all duration-200 cursor-pointer"
    >
      {isPending ? (
        <Loader />
      ) : (
